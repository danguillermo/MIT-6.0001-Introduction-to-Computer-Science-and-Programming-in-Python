...
# Time spent    : about 4 hours perhaps
...
def get_word_score(word, n):
    ...
    if len(word) == 0:
        ...
        for letter in word_l:
            word_points+= SCRABBLE_LETTER_VALUES[letter]
        
        second_c=max(1,7*len(word_l)-3*(n-len(word_l)))
        return(word_points*second_c)
...
def update_hand(hand, word):
    ...
    returns: dictionary (string -> int)
    """
    letters_hand=hand.keys()
    hand_copy=hand.copy()
    for letter in word.lower():
        if letter in letters_hand:
            if hand_copy.get(letter,False) > 1:
                hand_copy[letter] -= 1
            elif hand_copy.get(letter,False):
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while n > 0: # As long as there are still letters left in the hand:
        ...
        word = str(input('Enter a word, or "!!" to indicate you are finished: ')) # Ask user for input
...
def substitute_hand(hand, letter):
    ...
    returns: dictionary (string -> int)
    """
    hand_copy = hand.copy() #assuring no mutation
    vows = VOWELS  #Copying for modification
    cons = CONSONANTS
    
    if letter in hand.keys():
        if '*' in hand: #Deleting the '*' son its not selected for the subs
            del hand_copy['*']
    
        for let in hand_copy.keys(): #so the letter selected is not already in hand
            if let in VOWELS:
                vows.replace(let,'')
            else:
                cons.replace(let,'')
    
        del hand_copy[letter] #deleting letter
    
        if letter in VOWELS:
            Y = random.choice(vows)
        else:
            Y = random.choice(cons)
        
        hand_copy[Y] = hand.get(letter)
    
        if '*' in hand: #Adding the '*' so back to hand
            hand_copy['*'] = 1
    ...
    * Returns the total score for the series of hands
    ...
    while hands_number > 0:
        ...
        if Flag == 0: 
            ...
            user = str(input('Would you like to substitute a letter? '))
            if user.lower() == 'yes':
        ...
        if Flag_2 == 0:
            In = str(input('Would you like to reply the hand? '))
            print()
            if In.lower() == 'yes':
    ...
    return print("Total socre over all hands:",TOTAL_SCORE) # TO DO... Remove this line when you implement this function
